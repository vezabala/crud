...
import crud.com.web.rest.errors.BadRequestAlertException;
...
/**
 * REST controller for managing {@link crud.com.domain.Usuario}.
 */
@RestController
@RequestMapping("/api")
public class UsuarioResource {
    ...
    public ResponseEntity<UsuarioDTO> createUsuario(@Valid @RequestBody UsuarioDTO usuarioDTO) throws URISyntaxException {
        ...
        if (usuarioService.findByCedula(usuarioDTO.getCedula()).isPresent()) {
            throw new BadRequestAlertException("A new usuario cannot already have an CEDULA", ENTITY_NAME, "cedulaexists");
        }
    ...
    public ResponseEntity<UsuarioDTO> updateUsuario(@Valid @RequestBody UsuarioDTO usuarioDTO) throws URISyntaxException {
        ...
        Optional<UsuarioDTO> usuarioTemp = usuarioService.findByCedula(usuarioDTO.getCedula());
        if (usuarioTemp.isPresent() && !usuarioTemp.get().getId().equals(usuarioDTO.getId())) {
            throw new BadRequestAlertException("A new typeDocument cannot already have an CEDULA", ENTITY_NAME, "cedulaexists");
        }
        UsuarioDTO result = usuarioService.save(usuarioDTO);
    ...
    /**
     * Get all the typeDocuments by state.
     * @param estado state to search
     * @return the list of entities.
     */
    @GetMapping("/usuarios/estado/{estado}")
    public ResponseEntity<List<UsuarioDTO>> getAllusuariosByEstado(@PathVariable String estado) {
        log.debug("REST request to get a page of usuarios");
        List<UsuarioDTO> page = usuarioService.findAllByEstado(estado);
